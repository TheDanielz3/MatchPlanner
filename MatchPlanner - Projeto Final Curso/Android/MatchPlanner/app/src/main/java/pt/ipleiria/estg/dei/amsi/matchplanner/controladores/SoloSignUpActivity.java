...
public class SoloSignUpActivity extends AppCompatActivity
{
    ...
    {
        Soloprofile novoSoloProfile = new Soloprofile(0, firstname.getText().toString(), surnames.getText().toString(),
                email.getText().toString(), sex.getText().toString(), birthdate.getText().toString(),
        ...
        if(surnames.length() < 1 || surnames.length() > 50)
        {
        ...
        Soloprofile soloprofile = SingletonMatchPlanner.getInstance(getApplicationContext()).addSoloProfileDB(novoSoloProfile);
